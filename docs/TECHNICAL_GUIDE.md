...
3. **Universal LLM fallback** — a single structured-output call (`get_universal_sub_query_analyzer`, gpt-4o-mini) extracts companies, query type, requested years, sub-queries, and a `filing_type_hint` — all in one call, no extra round-trips.

All three paths also resolve **`filing_type`** (via `detect_filing_type_in_query()` — pure keyword heuristic, `None` if the query doesn't imply a specific type — search all types) and **`requested_fiscal_quarter`** (via `extract_fiscal_quarter_from_question()` — parses "Q1"/"first quarter" into a plain 1-4 integer, no calendar interpretation yet since the target ticker isn't resolved at this point).
...
**Filing-type-aware routing**: `hybrid_search()` accepts optional `filing_type`, `period_end_date`, and `fiscal_quarter` filters — all additive, all default to `None` (search everything), so a collection or query with no filing-type signal behaves exactly as it did before this was added.

**The 3-year comparative-window collapse** (segment/geographic queries spanning multiple years) branches on `filing_type`:
...
- Question rewriter preserves filing-type-implying language ("latest quarter", "recent announcement") rather than stripping it during query expansion.
...
- Web-search fallback query (in `nodes.py`'s `web_search()`) varies its SEC filing-type literal by detected/inferred `filing_type` instead of always assuming "10-K".
...
5. **`_is_direct_vectordb_mode()` bypass risk** — comparison/segment/geographic queries skipped grading and web-search fallback entirely, on a "10-K is authoritative" assumption that doesn't hold once 10-Q/8-K data (thinner per-document) flows through the same paths. Now the bypass only applies when the resolved `filing_type` is `None`/`"10-K"` (byte-for-byte unchanged behavior there); 10-Q/8-K queries fall through to the normal grade → web-fallback safety net instead. Verified with 9 direct test cases covering both branches.
...
8. **Env vars used in code but missing from `.env.example`**: `FRED_API_KEY` (macro ingestion raises without it), `CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET` (chart upload features degrade gracefully without them), `JWT_SECRET_KEY` (has a hardcoded insecure default in `app/auth/jwt.py` if unset — **must** be overridden before any real deployment), `SEC_USER_AGENT` (defaults to a placeholder contact string).
9. **Semantic cache (`rag/graph/semantic_cache.py`) is not wired into any live code path** — correctness-fixed (filter-aware cache key) in case it's adopted later, but currently dead code.
